...
router.get('/', (req, res) => {
  const queryText = 'SELECT * FROM "items" ORDER BY "id" DESC LIMIT 10';
...
router.post('/', (req, res) => {
  ...
  const queryText = `
    INSERT INTO "items" ("name", "note", "category", "quantity")
    VALUES ($1, $2, $3, $4) RETURNING id
  `;
  pool.query(queryText, [name, note, category, quantity])
    .then(() => res.sendStatus(201))
    .catch((err) => {
      console.error('Error in POST item:', err);
      res.sendStatus(500);
    });
});
...
router.put('/:id', (req, res) => {
  ...
  pool.query(queryText, [name, note, category, quantity, id])
    ...
    .catch((err) => {
      console.error('Error in PUT item:', err);
      res.sendStatus(500);
    });
});
